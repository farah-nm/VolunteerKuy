<nav class="bg-green-600 text-white py-4">
            ...
            <div class="hidden md:block">
                <div class="ml-4 flex items-center md:ml-6">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-green-200 hover:bg-green-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Logout</button>
                    </form>
                </div>
            </div>
            <div class="-mr-2 flex md:hidden">
                <button @click="open = ! open" class="bg-green-600 inline-flex items-center justify-center p-2 rounded-md text-green-200 hover:text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-green-600 focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
    ...
    <div class="md:hidden" id="mobile-menu">
        ...
        <div class="pt-4 pb-3 border-t border-green-700">
            <div class="px-4">
                <div class="font-medium text-green-200">{{ Auth::user()->name }}</div>
                <div class="font-medium text-sm text-green-300">{{ Auth::user()->email }}</div>
            </div>
            <div class="mt-3 space-y-1">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="block px-3 py-2 rounded-md text-base font-medium text-green-300 hover:text-white hover:bg-green-700">Logout</button>
                </form>
            </div>
        </div>
    </div>
